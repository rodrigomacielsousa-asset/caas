    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col">
          ...
          <div className="flex items-center gap-4 w-full md:w-auto">
            <div className="relative flex-1 md:w-72">
               ...
               <select 
                 ...
                 className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl pl-12 pr-6 py-4 text-xs font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-indigo-600 transition-all appearance-none"
               >
        ...
        {!selectedClientId ? (
             ...
             <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl w-full">
                {[
                  ...
                  <div key={f.label} className="p-6 bg-white dark:bg-slate-900 rounded-[32px] border border-slate-100 dark:border-slate-800 text-center space-y-3">
                     <f.icon className="w-6 h-6 text-indigo-600 mx-auto" />
          ...
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="bg-slate-900 rounded-[56px] p-10 text-white relative overflow-hidden shadow-2xl">
               <div className="absolute top-0 right-0 p-12 opacity-10 rotate-12 scale-150"><Zap className="w-64 h-64 text-indigo-400" /></div>
               <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
                  <div className="space-y-4">
                     <div className="flex items-center gap-3">
                        <span className="px-4 py-1 bg-indigo-600 text-white rounded-full text-[10px] font-black uppercase tracking-widest">Ativo</span>
                        <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest font-mono">{selectedClient?.cnpj}</span>
                     </div>
                  ...
                  <div className="flex gap-4">
                     <div className="p-6 bg-white/5 rounded-[32px] border border-white/10 text-center min-w-[140px]">
                        <p className="text-[9px] font-black text-indigo-400 uppercase tracking-widest mb-2">Saúde Fiscal</p>
                        <p className="text-2xl font-black text-emerald-400 italic">ALTA</p>
                     </div>
                     <div className="p-6 bg-white/5 rounded-[32px] border border-white/10 text-center min-w-[140px]">
                        <p className="text-[9px] font-black text-indigo-400 uppercase tracking-widest mb-2">Integridade</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
               {[
                 ...
                 { label: 'Saldo Projetado', val: `R$ ${stats.saldo.toLocaleString()}`, icon: DollarSign, color: 'text-indigo-600', bg: 'bg-indigo-50' },
                     ...
                     <motion.div key="p" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-10 space-y-12">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                           <div className="space-y-8">
                              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic flex items-center gap-2 underline decoration-indigo-600/30 underline-offset-4"><TrendingUp className="w-4 h-4" /> Desempenho Mensal</h4>
                              <div className="space-y-4">
                           ...
                           <div className="space-y-8">
                              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic flex items-center gap-2 underline decoration-indigo-600/30 underline-offset-4"><AlertTriangle className="w-4 h-4" /> Inconsistências Detectadas</h4>
                              <div className="bg-slate-900 rounded-[40px] p-8 space-y-6">
                                 ...
                                 <div className="flex gap-4 items-start">
                                    <div className="w-10 h-10 bg-indigo-500/20 rounded-xl flex items-center justify-center text-indigo-500 shrink-0"><Zap className="w-5 h-5" /></div>
                                    <div className="space-y-1">
                        ...
                        <div className="flex flex-col md:flex-row gap-4">
                           <button 
                             ...
                             className="px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-indigo-100 dark:shadow-none hover:bg-slate-900 transition-all flex items-center justify-center gap-3 group"
                           >
                              {isAnalyzing ? (
                        ...
                        <div className="flex flex-col lg:flex-row gap-10 border-t border-slate-100 dark:border-slate-800 pt-10">
                           <div className="w-full lg:w-96 space-y-6">
                                 ...
                                 <div className="flex items-center gap-6">
                                    <div className="w-24 h-24 rounded-full border-[6px] border-slate-100 dark:border-slate-800 flex items-center justify-center relative">
                                       <svg className="w-full h-full -rotate-90">
                                          <circle cx="48" cy="48" r="42" fill="none" strokeWidth="6" stroke="currentColor" strokeDasharray="264" strokeDashoffset={264 - (264 * (hubInsights?.score || (stats.saldo > 0 ? 88 : 42))) / 100} className="text-indigo-600 transition-all duration-1000" />
                                       </svg>
                              ...
                              <div className="bg-slate-950 rounded-[48px] p-8 text-white space-y-6 relative overflow-hidden">
                                 ...
                                 <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-widest italic tracking-widest relative z-10">IA Highlights</h4>
                                    ...
                                    {hubInsights?.insights ? hubInsights.insights.slice(0, 3).map((ins: any, idx: number) => (
                                      <div key={idx} className="space-y-2 translate-y-0 animate-in fade-in slide-in-from-top-2 duration-300">
                                         <div className="flex items-center gap-2">
                                            <div className={cn("w-1.5 h-4 rounded-full", ins.type === 'alerta' ? 'bg-rose-500' : (ins.type === 'oportunidade' ? 'bg-emerald-500' : 'bg-indigo-500'))} />
                           ...
                           <div className="flex-1 space-y-8">
                              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic flex items-center gap-2 underline decoration-indigo-600/30 underline-offset-4"><TrendingUp className="w-4 h-4" /> Próximos Passos</h4>
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                 {hubInsights?.proximosPassos ? hubInsights.proximosPassos.map((p: string, idx: number) => (
                                   <div key={idx} className="p-6 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[32px] flex items-center gap-4 group hover:border-indigo-600 transition-all">
                                      <div className="w-8 h-8 bg-slate-50 dark:bg-slate-800 rounded-xl flex items-center justify-center text-slate-400 group-hover:bg-indigo-600 group-hover:text-white transition-all"><ChevronRight className="w-4 h-4" /></div>
                   ...
                   {activeTab === 'transações' && (
                     <motion.div key="t" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col h-full">
                        <div className="p-8 flex justify-between items-center border-b border-slate-100 dark:border-slate-800">
                           ...
                           <button className="text-[10px] font-black text-indigo-600 uppercase tracking-widest flex items-center gap-2 hover:bg-indigo-50 p-2 rounded-xl transition-all">Baixar Ledger (CSV) <ChevronRight className="w-3 h-3" /></button>
                        </div>
                             ...
                             documents.map(d => (
                               <div key={d.id} className="p-6 bg-slate-50 dark:bg-slate-800 rounded-[32px] border border-slate-100 dark:border-slate-800 space-y-4 hover:shadow-lg transition-all group">
                                  <div className="flex justify-between items-start">
                                     <div className="w-10 h-10 bg-white dark:bg-slate-700 rounded-2xl flex items-center justify-center text-slate-400 group-hover:text-indigo-600 transition-colors">
                                        <FileText className="w-5 h-5" />
                                     </div>
                                     <span className="text-[9px] font-black text-indigo-600 bg-white dark:bg-slate-900 px-3 py-1 rounded-full uppercase tracking-widest italic">{d.type}</span>
                                  </div>
                                  <div>
                                  ...
                                  <div className="flex items-center justify-between pt-2 border-t border-slate-200 dark:border-slate-700">
                                     ...
                                     <button className="text-[9px] font-black text-indigo-600 uppercase tracking-widest p-2 hover:bg-white rounded-xl transition-all">Ver Doc</button>
                                  </div>
                               </div>
                             ))
                           )}
                     ...
                     <motion.div key="i" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-10 flex flex-col items-center justify-center space-y-8 min-h-[400px]">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-xl">
                           <div className="p-6 bg-slate-50 dark:bg-slate-800 rounded-[32px] border border-slate-100 dark:border-slate-800 space-y-2">
                              <p className="text-[10px] font-black text-indigo-600 uppercase">Cruzamento Tributário</p>
                              <p className="text-sm font-bold text-slate-900 dark:text-white">100% das notas fiscais conferem com os débitos bancários.</p>
                           </div>
                           <div className="p-6 bg-slate-50 dark:bg-slate-800 rounded-[32px] border border-slate-100 dark:border-slate-800 space-y-2">
                              <p className="text-[10px] font-black text-indigo-600 uppercase">Validação de Saldo</p>
      ...
      <div className="max-w-7xl mx-auto w-full px-8 py-10 mt-10 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center text-slate-400 font-medium pb-20">
         <div className="flex items-center gap-3 text-xs italic">
            <Zap className="w-4 h-4 text-indigo-600" /> Data Hub Core v4.1 (Enterprise Layer)
         </div>
         <div className="flex gap-8 text-[10px] font-black uppercase tracking-widest">
            <a href="#" className="hover:text-indigo-600 transition-colors italic">Integridade Documental</a>
            <a href="#" className="hover:text-indigo-600 transition-colors italic">Ledger API</a>
            <a href="#" className="hover:text-indigo-600 transition-colors italic">Segurança de Dados</a>
         </div>
      </div>
    </div>
  );
