        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
            <div className="flex items-center gap-8">
              <div className="w-24 h-24 bg-indigo-600 rounded-[36px] flex items-center justify-center text-white text-3xl font-black shadow-2xl shadow-indigo-100 dark:shadow-none relative">
          ...
          <div className="lg:col-span-2 space-y-12">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-6">
              <h2 className="text-xl font-black text-slate-900 dark:text-white flex items-center gap-3 tracking-tighter uppercase italic">
                Meus Sistemas <span className="text-[10px] font-black bg-indigo-600 text-white px-3 py-1 rounded-full not-italic tracking-widest">{accessibleProducts.length}</span>
              </h2>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               {accessibleProducts.map((p) => (
                 <motion.div
                  whileHover={{ y: -8 }}
                  key={p.id}
                  className="bg-white dark:bg-slate-900 p-10 rounded-[48px] border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-2xl hover:shadow-indigo-100 dark:hover:shadow-none transition-all group relative overflow-hidden"
                 >
                   <div className="absolute top-0 right-0 p-8 opacity-5 -translate-y-4 translate-x-4"><Zap className="w-32 h-32" /></div>
                   
                   <div className="flex justify-between items-start mb-8 relative z-10">
                      <div className="w-16 h-16 bg-slate-50 dark:bg-slate-800 rounded-3xl flex items-center justify-center text-slate-400 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-500 shadow-inner">
                   ...
                   <div className="mt-10 pt-8 border-t border-slate-50 dark:border-slate-800 flex items-center justify-between relative z-10">
                      ...
                      <Link 
                        ...
                        className="px-6 py-3 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 transition-all flex items-center gap-2 group/btn"
                      >
            ...
            {lockedProducts.length > 0 && (
              <div className="space-y-8 pt-12">
                 <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic flex items-center gap-2 underline decoration-indigo-600/30 underline-offset-4">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6 opacity-60 hover:opacity-100 transition-opacity">
                    {lockedProducts.map(p => (
                      <div key={p.id} className="p-8 bg-slate-50 dark:bg-slate-900/50 rounded-[40px] border border-dashed border-slate-200 dark:border-slate-800 flex justify-between items-center group">
                         ...
                         <Link to="/pricing" className="w-10 h-10 bg-white dark:bg-slate-800 rounded-xl flex items-center justify-center text-indigo-600 shadow-sm hover:bg-indigo-600 hover:text-white transition-all"><ArrowRight className="w-4 h-4" /></Link>
                      </div>
                    ))}
                 </div>
              </div>
          ...
          <div className="space-y-12">
            <div className="bg-slate-950 rounded-[56px] p-12 text-white space-y-10 relative overflow-hidden shadow-2xl">
               ...
               <div className="space-y-4 relative z-10">
                  <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-widest italic">Hub da Empresa</h4>
               ...
               <div className="space-y-6 relative z-10">
                  {[
                    ...
                    { label: 'Eficiência IA', val: '84%', icon: Zap, color: 'text-indigo-400' }
                  ].map(s => (
               ...
               <button className="w-full py-6 bg-white text-slate-900 rounded-[28px] font-black text-xs uppercase tracking-widest hover:bg-indigo-50 transition-all shadow-xl relative z-10">
                  Ver Relatórios Hub
               </button>
            </div>
